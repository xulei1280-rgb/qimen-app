...
assert(theoreticalBaselineDoc.includes('ZP-TB-1984-2044-D11-H12-v3') && theoreticalBaselineDoc.includes('23,916') && theoreticalBaselineDoc.includes('不使用名人、朋友'), 'the theoretical baseline document should freeze version, sample count, and no-real-person boundary');
...
assert(referencePattern.analysisMeta.engineVersion === 'BAZI-PATTERN-2026.07.13.3' && referencePattern.analysisMeta.ruleVersion === 'ZP-2026.07.13.7' && referencePattern.analysisMeta.baselineVersion === 'ZP-TB-1984-2044-D11-H12-v3' && referencePattern.analysisMeta.baselineScope.includes('不使用真人结果标签'), `pattern results should expose the frozen engine/rule/baseline versions and theoretical-only scope, got ${JSON.stringify(referencePattern.analysisMeta)}`);
...
assert(theoreticalBaseline.version === 'ZP-TB-1984-2044-D11-H12-v3' && theoreticalBaseline.config.stepDays === 11 && theoreticalBaseline.config.hours.length === 12 && theoreticalBaseline.config.deduplicate === 'fourPillars' && theoreticalBaseline.config.trueSolarCorrection === false, `the frozen theoretical grid should expose its complete sampling contract, got ${JSON.stringify(theoreticalBaseline)}`);
assert(Object.values(theoreticalBaseline.histogram).reduce((sum, count) => sum + count, 0) === 23916 && theoreticalBaseline.stats.uniqueCount === 23916 && theoreticalBaseline.stats.quantiles.p20 === 37 && theoreticalBaseline.stats.quantiles.p80 === 68, `the frozen histogram and quantile boundaries should match the reproducible v3 baseline, got ${JSON.stringify(theoreticalBaseline.stats)}`);
assert(JSON.stringify(theoreticalBaseline.stats.candidateGradeCounts) === JSON.stringify({偏低:1156,中等:3718,偏高:14317,高:3523,顶级:1202}) && JSON.stringify(theoreticalBaseline.stats.gradeCounts) === JSON.stringify({偏低:1135,中等:3803,偏高:14552,高:4044,顶级:382}) && theoreticalBaseline.stats.gradePercentages.顶级 === 1.6, `the frozen candidate and hard-gated grade distributions should match the full theoretical grid, got ${JSON.stringify(theoreticalBaseline.stats)}`);
...
assert(crypto.createHash('sha256').update(JSON.stringify(theoreticalBaseline.histogram)).digest('hex') === '38fef219e6ba186ce8e82242b2dfced91fa29e13dad73f5ceac04e332548f5cf', 'the frozen theoretical histogram hash should not drift without a new baseline version');
...
assert(!scriptContext.patternLevelText(referenceBazi).includes('理论结构位置：') && !scriptContext.patternLevelText(referenceBazi).includes('原始结构分') && !scriptContext.patternLevelText(referenceBazi).includes('ZP-') && scriptContext.patternLevelText(referenceBazi).startsWith(referencePattern.patternLevelGrade) && scriptContext.patternLevelText(referenceBazi).endsWith('理论位置：P'+referencePattern.patternPercentile), `the page level output should keep the direct grade, append only the P position, and suppress internal scoring details, got ${scriptContext.patternLevelText(referenceBazi)}`);
assert(scriptContext.patternLevelText(referenceBazi, true).includes('理论结构位置：') && scriptContext.patternLevelText(referenceBazi, true).includes('P'+referencePattern.patternPercentile), `technical level context should retain the frozen theoretical percentile, got ${scriptContext.patternLevelText(referenceBazi, true)}`);
