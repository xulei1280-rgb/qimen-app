...
assert(theoreticalBaselineDoc.includes('ZP-TB-1984-2044-D11-H12-v6') && theoreticalBaselineDoc.includes('23,916') && theoreticalBaselineDoc.includes('不使用名人、朋友'), 'the theoretical baseline document should freeze version, sample count, and no-real-person boundary');
...
assert(referencePattern.analysisMeta.engineVersion === 'BAZI-PATTERN-2026.07.14.3' && referencePattern.analysisMeta.ruleVersion === 'ZP-2026.07.14.2' && referencePattern.analysisMeta.baselineVersion === 'ZP-TB-1984-2044-D11-H12-v6' && referencePattern.analysisMeta.scoreModelVersion === 'ZP-SCORE-2026.07.14-v2' && referencePattern.analysisMeta.baselineScope.includes('不使用真人结果标签'), `pattern results should expose the frozen engine/rule/baseline/score versions and theoretical-only scope, got ${JSON.stringify(referencePattern.analysisMeta)}`);
...
assert(theoreticalBaseline.version === 'ZP-TB-1984-2044-D11-H12-v6' && theoreticalBaseline.scoreModelVersion === 'ZP-SCORE-2026.07.14-v2' && theoreticalBaseline.config.stepDays === 11 && theoreticalBaseline.config.hours.length === 12 && theoreticalBaseline.config.deduplicate === 'fourPillars' && theoreticalBaseline.config.trueSolarCorrection === false, `the frozen theoretical grid should expose its complete sampling contract, got ${JSON.stringify(theoreticalBaseline)}`);
assert(Object.values(theoreticalBaseline.histogram).reduce((sum, count) => sum + count, 0) === 23916 && theoreticalBaseline.stats.uniqueCount === 23916 && theoreticalBaseline.stats.scoreRange.min === 23 && theoreticalBaseline.stats.scoreRange.max === 89 && theoreticalBaseline.stats.quantiles.p20 === 33 && theoreticalBaseline.stats.quantiles.p80 === 74, `the frozen histogram and quantile boundaries should match the reproducible v6 baseline, got ${JSON.stringify(theoreticalBaseline.stats)}`);
assert(JSON.stringify(theoreticalBaseline.stats.candidateGradeCounts) === JSON.stringify({偏低:1063,中等:3272,偏高:14872,高:3657,顶级:1052}) && JSON.stringify(theoreticalBaseline.stats.gradeCounts) === JSON.stringify({偏低:1063,中等:4222,偏高:14173,高:3623,顶级:835}) && theoreticalBaseline.stats.gradePercentages.顶级 === 3.49, `the frozen candidate and hard-gated grade distributions should match the full theoretical grid, got ${JSON.stringify(theoreticalBaseline.stats)}`);
...
assert(crypto.createHash('sha256').update(JSON.stringify(theoreticalBaseline.histogram)).digest('hex') === '434ef29ac1109fe9bcc337f79bad12dad5ad229c8f40a0a2bb406460370b8c60', 'the frozen theoretical histogram hash should not drift without a new baseline version');
...
if (process.env.BAZI_FULL_PATTERN_BASELINE === '1') {
  ...
  assert(JSON.stringify(rebuiltBaseline.histogram) === JSON.stringify(theoreticalBaseline.histogram) && JSON.stringify(rebuiltBaseline.quantiles) === JSON.stringify(theoreticalBaseline.stats.quantiles) && countsMatch(rebuiltBaseline.candidateGradeCounts, theoreticalBaseline.stats.candidateGradeCounts) && countsMatch(rebuiltBaseline.gradeCounts, theoreticalBaseline.stats.gradeCounts), `full theoretical baseline rebuild should reproduce the frozen v6 constants, got ${JSON.stringify(rebuiltBaseline)}`);
}
...
assert(killSealPattern.elementPhenomena[0].category === '五行气象与偏枯病象' && killSealPattern.elementPhenomena[0].termType === '五行气象与偏枯病象' && killSealPattern.elementPhenomena[0].sourceSystem === '结构诊断与校正层' && killSealPattern.elementPhenomena[0].target.role === '日主本体' && killSealPattern.elementPhenomena[0].ruleId === 'ZP-QX-001' && killSealPattern.elementPhenomena[0].ruleVersion === 'ZP-2026.07.14.2', `phenomenon should remain separate from the formal pattern and expose its type, object, source layer, rule, and version, got ${JSON.stringify(killSealPattern.elementPhenomena[0])}`);
...
outputExcessCases.forEach((item) => {
  ...
  assert(phenomenon && phenomenon.ruleId === item.ruleId && phenomenon.ruleVersion === 'ZP-2026.07.14.2' && phenomenon.category === '五行气象与泄身反伤病象' && phenomenon.termType === '五行气象与偏枯病象' && phenomenon.status === '明确成立', `${item.name} should form only after the shared output-excess conditions are met and remain in the versioned phenomenon term layer, got ${JSON.stringify(result.elementPhenomena)}`);
...
resourceExcessCases.forEach((item) => {
  const result = scriptContext.BaziEngine.analyzePattern({ pillars: item.pillars, dayStem: item.pillars.day[0] });
  const phenomenon = result.elementPhenomena.find((x) => x.name === item.name);
  assert(phenomenon && phenomenon.ruleId === item.ruleId && phenomenon.ruleVersion === 'ZP-2026.07.14.2' && phenomenon.category === '五行气象与生扶太过病象' && phenomenon.status === '明显倾向', `${item.name} should require excessive resource, a weakly rooted recipient, and a versioned strict rule, got ${JSON.stringify(result.elementPhenomena)}`);
...
positivePhenomenonCases.forEach((item) => {
  ...
  assert(positive && positive.status === '明确成立' && positive.evidence.length === 3 && positive.boundary && positive.ruleVersion === 'ZP-2026.07.14.2', `${item.name} should require season, visibility, roots, balance, and a stated counter-boundary, got ${JSON.stringify(result.positivePhenomena)}`);
