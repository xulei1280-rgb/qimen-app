...
assert(theoreticalBaselineDoc.includes('ZP-TB-1984-2044-D11-H12-v8') && theoreticalBaselineDoc.includes('23,916') && theoreticalBaselineDoc.includes('不使用名人、朋友'), 'the theoretical baseline document should freeze version, sample count, and no-real-person boundary');
...
assert(html.includes('<script src="bazi-engine.js?v=20260715.1"></script>'), 'bazi page should load the current bazi-engine.js asset revision');
...
assert(referencePattern.analysisMeta.engineVersion === 'BAZI-PATTERN-2026.07.15.1' && referencePattern.analysisMeta.ruleVersion === 'ZP-2026.07.14.3' && referencePattern.analysisMeta.baselineVersion === 'ZP-TB-1984-2044-D11-H12-v8' && referencePattern.analysisMeta.scoreModelVersion === 'ZP-SCORE-2026.07.15-v3' && referencePattern.analysisMeta.baselineScope.includes('不使用真人结果标签'), `pattern results should expose the frozen engine/rule/baseline/score versions and theoretical-only scope, got ${JSON.stringify(referencePattern.analysisMeta)}`);
...
assert(theoreticalBaseline.version === 'ZP-TB-1984-2044-D11-H12-v8' && theoreticalBaseline.scoreModelVersion === 'ZP-SCORE-2026.07.15-v3' && theoreticalBaseline.config.stepDays === 11 && theoreticalBaseline.config.hours.length === 12 && theoreticalBaseline.config.deduplicate === 'fourPillars' && theoreticalBaseline.config.trueSolarCorrection === false, `the frozen theoretical grid should expose its complete sampling contract, got ${JSON.stringify(theoreticalBaseline)}`);
assert(Object.values(theoreticalBaseline.histogram).reduce((sum, count) => sum + count, 0) === 23916 && theoreticalBaseline.stats.uniqueCount === 23916 && theoreticalBaseline.stats.scoreRange.min === 40 && theoreticalBaseline.stats.scoreRange.max === 89 && theoreticalBaseline.stats.quantiles.p20 === 52 && theoreticalBaseline.stats.quantiles.p80 === 75, `the frozen histogram and quantile boundaries should match the reproducible v8 baseline, got ${JSON.stringify(theoreticalBaseline.stats)}`);
assert(JSON.stringify(theoreticalBaseline.stats.candidateGradeCounts) === JSON.stringify({偏低:1269,中等:3865,偏高:13743,高:3846,顶级:1193}) && JSON.stringify(theoreticalBaseline.stats.gradeCounts) === JSON.stringify({偏低:1269,中等:4534,偏高:13307,高:3916,顶级:890}) && theoreticalBaseline.stats.gradePercentages.顶级 === 3.72, `the frozen candidate and hard-gated grade distributions should match the full theoretical grid, got ${JSON.stringify(theoreticalBaseline.stats)}`);
...
if (process.env.BAZI_FULL_PATTERN_BASELINE === '1') {
  ...
  assert(JSON.stringify(rebuiltBaseline.histogram) === JSON.stringify(theoreticalBaseline.histogram) && JSON.stringify(rebuiltBaseline.quantiles) === JSON.stringify(theoreticalBaseline.stats.quantiles) && countsMatch(rebuiltBaseline.candidateGradeCounts, theoreticalBaseline.stats.candidateGradeCounts) && countsMatch(rebuiltBaseline.gradeCounts, theoreticalBaseline.stats.gradeCounts), `full theoretical baseline rebuild should reproduce the frozen v8 constants, got ${JSON.stringify(rebuiltBaseline)}`);
