...
assert(theoreticalBaselineDoc.includes('ZP-TB-1984-2044-D11-H12-v7') && theoreticalBaselineDoc.includes('23,916') && theoreticalBaselineDoc.includes('不使用名人、朋友'), 'the theoretical baseline document should freeze version, sample count, and no-real-person boundary');
...
assert(html.includes('<script src="bazi-engine.js?v=20260714.4"></script>'), 'bazi page should load the current bazi-engine.js asset revision');
...
assert(theoreticalBaseline.version === 'ZP-TB-1984-2044-D11-H12-v7' && theoreticalBaseline.scoreModelVersion === 'ZP-SCORE-2026.07.14-v2' && theoreticalBaseline.config.stepDays === 11 && theoreticalBaseline.config.hours.length === 12 && theoreticalBaseline.config.deduplicate === 'fourPillars' && theoreticalBaseline.config.trueSolarCorrection === false, `the frozen theoretical grid should expose its complete sampling contract, got ${JSON.stringify(theoreticalBaseline)}`);
assert(Object.values(theoreticalBaseline.histogram).reduce((sum, count) => sum + count, 0) === 23916 && theoreticalBaseline.stats.uniqueCount === 23916 && theoreticalBaseline.stats.scoreRange.min === 23 && theoreticalBaseline.stats.scoreRange.max === 89 && theoreticalBaseline.stats.quantiles.p20 === 33 && theoreticalBaseline.stats.quantiles.p80 === 75, `the frozen histogram and quantile boundaries should match the reproducible v7 baseline, got ${JSON.stringify(theoreticalBaseline.stats)}`);
assert(JSON.stringify(theoreticalBaseline.stats.candidateGradeCounts) === JSON.stringify({偏低:1428,中等:3203,偏高:14375,高:3783,顶级:1127}) && JSON.stringify(theoreticalBaseline.stats.gradeCounts) === JSON.stringify({偏低:1428,中等:4208,偏高:13600,高:3790,顶级:890}) && theoreticalBaseline.stats.gradePercentages.顶级 === 3.72, `the frozen candidate and hard-gated grade distributions should match the full theoretical grid, got ${JSON.stringify(theoreticalBaseline.stats)}`);
...
assert(crypto.createHash('sha256').update(JSON.stringify(theoreticalBaseline.histogram)).digest('hex') === '4a80f106a5dc816b99a4992709f5f8c9b979b233bec2293e731bead74a7b4f64', 'the frozen theoretical histogram hash should not drift without a new baseline version');
...
if (process.env.BAZI_FULL_PATTERN_BASELINE === '1') {
  ...
  assert(JSON.stringify(rebuiltBaseline.histogram) === JSON.stringify(theoreticalBaseline.histogram) && JSON.stringify(rebuiltBaseline.quantiles) === JSON.stringify(theoreticalBaseline.stats.quantiles) && countsMatch(rebuiltBaseline.candidateGradeCounts, theoreticalBaseline.stats.candidateGradeCounts) && countsMatch(rebuiltBaseline.gradeCounts, theoreticalBaseline.stats.gradeCounts), `full theoretical baseline rebuild should reproduce the frozen v7 constants, got ${JSON.stringify(rebuiltBaseline)}`);
