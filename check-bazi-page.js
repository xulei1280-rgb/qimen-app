...
assert(theoreticalBaselineDoc.includes('ZP-TB-1984-2044-D11-H12-v4') && theoreticalBaselineDoc.includes('23,916') && theoreticalBaselineDoc.includes('不使用名人、朋友'), 'the theoretical baseline document should freeze version, sample count, and no-real-person boundary');
...
assert(referencePattern.analysisMeta.engineVersion === 'BAZI-PATTERN-2026.07.14.1' && referencePattern.analysisMeta.ruleVersion === 'ZP-2026.07.13.7' && referencePattern.analysisMeta.baselineVersion === 'ZP-TB-1984-2044-D11-H12-v4' && referencePattern.analysisMeta.scoreModelVersion === 'ZP-SCORE-2026.07.14-v1' && referencePattern.analysisMeta.baselineScope.includes('不使用真人结果标签'), `pattern results should expose the frozen engine/rule/baseline/score versions and theoretical-only scope, got ${JSON.stringify(referencePattern.analysisMeta)}`);
...
assert(referenceScoreLedger.modelVersion === referencePattern.analysisMeta.scoreModelVersion && referenceScoreLedger.method.includes('五维线性加权') && referenceScoreLedger.method.includes('格名不直接加分') && referenceScoreLedger.method.includes('不使用现实人物结果标签'), `raw score should expose the agreed score model and boundary, got ${JSON.stringify(referenceScoreLedger)}`);
assert(JSON.stringify(referenceScoreLedger.dimensions.map((x) => [x.key, x.weight])) === JSON.stringify([['formation',35],['flow',20],['clarity',15],['remedy',20],['balance',10]]) && referenceScoreLedger.dimensions.every((x) => typeof x.score === 'number' && typeof x.contribution === 'number' && x.evidence), `raw score should expose five attributable weighted dimensions, got ${JSON.stringify(referenceScoreLedger.dimensions)}`);
...
assert(theoreticalBaseline.version === 'ZP-TB-1984-2044-D11-H12-v4' && theoreticalBaseline.scoreModelVersion === 'ZP-SCORE-2026.07.14-v1' && theoreticalBaseline.config.stepDays === 11 && theoreticalBaseline.config.hours.length === 12 && theoreticalBaseline.config.deduplicate === 'fourPillars' && theoreticalBaseline.config.trueSolarCorrection === false, `the frozen theoretical grid should expose its complete sampling contract, got ${JSON.stringify(theoreticalBaseline)}`);
assert(Object.values(theoreticalBaseline.histogram).reduce((sum, count) => sum + count, 0) === 23916 && theoreticalBaseline.stats.uniqueCount === 23916 && theoreticalBaseline.stats.scoreRange.min === 32 && theoreticalBaseline.stats.scoreRange.max === 83 && theoreticalBaseline.stats.quantiles.p20 === 47 && theoreticalBaseline.stats.quantiles.p80 === 72, `the frozen histogram and quantile boundaries should match the reproducible v4 baseline, got ${JSON.stringify(theoreticalBaseline.stats)}`);
assert(JSON.stringify(theoreticalBaseline.stats.candidateGradeCounts) === JSON.stringify({偏低:1704,中等:3076,偏高:14380,高:3858,顶级:898}) && JSON.stringify(theoreticalBaseline.stats.gradeCounts) === JSON.stringify({偏低:1704,中等:3076,偏高:14775,高:3984,顶级:377}) && theoreticalBaseline.stats.gradePercentages.顶级 === 1.58, `the frozen candidate and hard-gated grade distributions should match the full theoretical grid, got ${JSON.stringify(theoreticalBaseline.stats)}`);
...
assert(crypto.createHash('sha256').update(JSON.stringify(theoreticalBaseline.histogram)).digest('hex') === '55dfb29a81ac3bcbfa308b60bbb95f11c008735e46d1485308a4376aa2158419', 'the frozen theoretical histogram hash should not drift without a new baseline version');
...
if (process.env.BAZI_FULL_PATTERN_BASELINE === '1') {
  ...
  assert(JSON.stringify(rebuiltBaseline.histogram) === JSON.stringify(theoreticalBaseline.histogram) && JSON.stringify(rebuiltBaseline.quantiles) === JSON.stringify(theoreticalBaseline.stats.quantiles) && countsMatch(rebuiltBaseline.candidateGradeCounts, theoreticalBaseline.stats.candidateGradeCounts) && countsMatch(rebuiltBaseline.gradeCounts, theoreticalBaseline.stats.gradeCounts), `full theoretical baseline rebuild should reproduce the frozen v4 constants, got ${JSON.stringify(rebuiltBaseline)}`);
}
...
assert(killSealPattern.elementPhenomena[0].category === '五行气象与偏枯病象' && killSealPattern.elementPhenomena[0].termType === '五行气象与偏枯病象' && killSealPattern.elementPhenomena[0].sourceSystem === '结构诊断与校正层' && killSealPattern.elementPhenomena[0].target.role === '日主本体' && killSealPattern.elementPhenomena[0].ruleId === 'ZP-QX-001' && killSealPattern.elementPhenomena[0].ruleVersion === 'ZP-2026.07.13.7', `phenomenon should remain separate from the formal pattern and expose its type, object, source layer, rule, and version, got ${JSON.stringify(killSealPattern.elementPhenomena[0])}`);
...
outputExcessCases.forEach((item) => {
  ...
  assert(phenomenon && phenomenon.ruleId === item.ruleId && phenomenon.ruleVersion === 'ZP-2026.07.13.7' && phenomenon.category === '五行气象与泄身反伤病象' && phenomenon.termType === '五行气象与偏枯病象' && phenomenon.status === '明确成立', `${item.name} should form only after the shared output-excess conditions are met and remain in the versioned phenomenon term layer, got ${JSON.stringify(result.elementPhenomena)}`);
...
resourceExcessCases.forEach((item) => {
  const result = scriptContext.BaziEngine.analyzePattern({ pillars: item.pillars, dayStem: item.pillars.day[0] });
  const phenomenon = result.elementPhenomena.find((x) => x.name === item.name);
  assert(phenomenon && phenomenon.ruleId === item.ruleId && phenomenon.ruleVersion === 'ZP-2026.07.13.7' && phenomenon.category === '五行气象与生扶太过病象' && phenomenon.status === '明显倾向', `${item.name} should require excessive resource, a weakly rooted recipient, and a versioned strict rule, got ${JSON.stringify(result.elementPhenomena)}`);
...
assert(scoreDimension(clearSealRescuePattern, 'clarity').score === 50 && scoreDimension(clearSealRescuePattern, 'clarity').evidence.includes('已由病药维度结算'), `a disease-owned clarity defect should not be deducted twice across clarity and remedy dimensions, got ${JSON.stringify(scoreDimension(clearSealRescuePattern, 'clarity'))}`);
...
assert(!strictInjurySealPattern.patternRawScoreBreakdown.ledger.some((item) => /严格成格|特殊格局/.test(item.name)) && scoreDimension(strictInjurySealPattern, 'formation').score === 85, `strict pattern names should affect hard gates but must not receive a direct raw-score bonus, got ${JSON.stringify(strictInjurySealPattern.patternRawScoreBreakdown)}`);
...
positivePhenomenonCases.forEach((item) => {
  ...
  assert(positive && positive.status === '明确成立' && positive.evidence.length === 3 && positive.boundary && positive.ruleVersion === 'ZP-2026.07.13.7', `${item.name} should require season, visibility, roots, balance, and a stated counter-boundary, got ${JSON.stringify(result.positivePhenomena)}`);
